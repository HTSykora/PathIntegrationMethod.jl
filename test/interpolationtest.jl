...
f_itp = ProbabilityDensityFunction(Float64,[GridAxis(et,start,stop,num; interpolation = itp) for (et, start, stop, num) in grid_dat]...; f = f)
...
f_itp = ProbabilityDensityFunction([GridAxis(start,stop,num; interpolation = itp) for (start, stop, num) in grid_dat]...; f = f)
xs = [LinRange(start,stop, 2(num - 1)) for (start,stop,num) in grid_dat]
