...
struct PDGrid{N,k,T,xT,pT,ξT,gridT} <: AbstractArray{T,N}
    xs::xT
    p::pT
    p_temp::pT
    ξ_temp::ξT
...
function PDGrid(sde::AbstractSDE{N,k},xs...; Q_initialize = true, kwargs...) where {N,k}
    ...
    if xs isa NTuple{N,Axis{<:LinearInterpolation{true}}} where N
        ...
        return PDGrid{N,k,eltype(p),typeof(xs),typeof(p),typeof(p),typeof(grid)}(xs, p, similar(p), ξ_temp, grid)
    else
        return PDGrid{N,k,eltype(p),typeof(xs),typeof(p),typeof(p),Nothing}(xs, p, similar(p), ξ_temp, nothing)
    end
end
