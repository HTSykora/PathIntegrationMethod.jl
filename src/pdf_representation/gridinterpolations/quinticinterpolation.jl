...
function quinticinterpolation_weights_2!(vals, dx, Δ) 
    ...
    vals[1] = (δ⁴+ 11δ²)*0.25*_6 - (δ³+δ)*0.25
    vals[2] = (5δ³ - δ⁴ - 5δ² - 5δ)*_6 + one(δ)
    vals[3] =  0.25*(δ⁴ + δ²) - δ³ + 1.5δ
    vals[4] = (δ²-δ⁴)*_6 + 0.5*(δ³ - δ)
    vals[5] = (δ⁴-δ²)*0.25*_6 + (δ-δ³)*0.5*_6
...
function quinticinterpolation_weights_e1!(vals, dx, Δ) 
    ...
    vals[1] = zero(δ)
    vals[2] = (δ⁴ - δ²)*0.25*_6 + (δ³ - δ)*0.5*_6
    vals[3] = (δ² - δ⁴)*_6 + (δ-δ³)*0.5
    vals[4] = (δ⁴ + δ²)*0.25 + δ³ - 1.5δ
    vals[5] = 5*(δ - δ³ - δ²)*_6 - δ⁴*_6 + one(δ)
    vals[6] = (δ⁴ + 11δ²)*0.25*_6 + 0.25*(δ³ + δ)
...
function quinticinterpolation_weights_e2!(vals, dx, Δ) 
    ...
    _3= one(δ)/typeof(δ)(3);
    vals[1] = zero(δ)
    vals[2] = (δ⁴ - δ²)*0.125*_3 + (δ - δ³)*0.25*_3
    vals[3] = (δ³-δ⁴)*0.5*_3 + (δ² - δ)*2*_3
    vals[4] = 0.25δ⁴ - 1.25δ² + one(δ)
    vals[5] = (δ² + δ)*2*_3 - (δ⁴+δ³)*0.5*_3
    vals[6] = (δ⁴ - δ²)*0.125*_3 + (δ³ - δ)*0.25*_3
