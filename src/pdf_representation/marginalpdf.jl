...
function initialise_mPDF(pdf,IDs)
    [initialise_mPDF(pdf,_IDs) for _IDs in IDs]
end
function initialise_mPDF(pdf::InterpolatedFunction{T,N},IDs::Union{Integer,NTuple{n,<:Integer}}) where {T,N}
...
function get_mpdf_intdims(pdf::InterpolatedFunction{T,N},ID::Integer) where N
    @assert ID ≤ N "ID > $N"
    @assert 0 < ID "ID < 0"
    
    Tuple(i for i in 1:N if i ≂̸ ID)
end
function get_mpdf_intdims(pdf::InterpolatedFunction{T,N},IDs::NTuple{n,Integer}) where {n,N}
...
function get_mpdf0(pdf,ID::NTuple{n,Integer})
...
function update_mPDF!(mpdf::MarginalPDF,pdf::InterpolatedFunction)
    ...
    mpdf.p0 .= sum(temp, dims=mpdf.dims)
    nothing
end
# Utils
